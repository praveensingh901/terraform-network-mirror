...
cd ${mirror_dir}
for row in $(echo -e $providers | jq -r '.[] | [.namespace, .name, .versions] | @base64'); do
  echo "===e======"
  echo $row
  echo "===d======"
  _namespace() {
    echo -e $row |tr -d '\r' | base64 --decode | jq -r .[0]
  }
  _name() {
    echo -e $row|tr -d '\r'  | base64 --decode | jq -r .[1]
  }
  _versions() {
    echo -e $row|tr -d '\r'  | base64 --decode | jq -r .[2]
  }
  echo $(_name) $(_versions)
  for version in $(_versions | jq -r .[] |tr -d '\r' ); do
    ns=$(_namespace)
    n=$(_name)
    v=${version}
